...
if [ "$GITHUB_REF" == "refs/heads/windows_build" ]; then
    if [[ $OSTYPE == 'darwin'* ]]; then
      ...
      LATEST_PUBLISHED_VERSION=$(curl -vL https://repo1.maven.org/maven2/io/arrow-kt/arrow-core/maven-metadata.xml | tac | tac | grep -oP '<latest>\K[^<]*')
    fi
