...
object MonadLaws {
  ...
  private fun <F> Monad<F>.leftIdentity(G: Gen<Kind<F, Int>>, EQ: Eq<Kind<F, Int>>): Unit =
    forAll(Gen.functionAToB(G), Gen.int()) { f: (Int) -> Kind<F, Int>, a: Int ->
      just(a).flatMap(f).equalUnderTheLaw(f(a), EQ)
    }

  private fun <F> Monad<F>.rightIdentity(G: Gen<Kind<F, Int>>, EQ: Eq<Kind<F, Int>>): Unit =
    forAll(G) { fa: Kind<F, Int> ->
      fa.flatMap { just(it) }.equalUnderTheLaw(fa, EQ)
    }

  private fun <F> Monad<F>.stackSafety(iter: Int = 5000, EQ: Eq<Kind<F, Int>>) {
    val res = tailRecM(0) { i -> just(if (i < iter) Left(i + 1) else Right(i)) }
    res.equalUnderTheLaw(just(iter), EQ)
  }

  private fun <F> Monad<F>.monadComprehensions(EQ: Eq<Kind<F, Int>>): Unit =
  ...
  private fun <F> Monad<F>.derivedApConsistent(GK: GenK<F>, AP: Apply<F>, EQ: Eq<Kind<F, Int>>): Unit =
    forAll(GK.genK(Gen.int()), GK.genK(Gen.functionAToB<Int, Int>(Gen.int()))) { fa, ff ->
      AP.run { fa.ap(ff) }.equalUnderTheLaw(fa.ap(ff), EQ)
    }

  private fun <F> Monad<F>.derivedFollowedByConsistent(GK: GenK<F>, AP: Apply<F>, EQ: Eq<Kind<F, Int>>): Unit =
    forAll(GK.genK(Gen.int()), GK.genK(Gen.int())) { fa, fb ->
      AP.run { fa.followedBy(fb) }.equalUnderTheLaw(fa.followedBy(fb), EQ)
    }

  private fun <F> Monad<F>.derivedApTapConsistent(GK: GenK<F>, AP: Apply<F>, EQ: Eq<Kind<F, Int>>): Unit =
    forAll(GK.genK(Gen.int()), GK.genK(Gen.int())) { fa, fb ->
      AP.run { fa.apTap(fb) }.equalUnderTheLaw(fa.apTap(fb), EQ)
    }

  private fun <F> Monad<F>.derivedFlattenConsistent(GK: GenK<F>, EQ: Eq<Kind<F, Int>>): Unit =
    forAll(GK.genK(GK.genK(Gen.int()))) { fa: Kind<F, Kind<F, Int>> ->
      fa.flatten().equalUnderTheLaw(fa.flatMap(::identity), EQ)
    }

  private fun <F> Monad<F>.derivedFollowedByEvalConsistent(GK: GenK<F>, EQ: Eq<Kind<F, Int>>): Unit =
    forAll(GK.genK(Gen.int()), GK.genK(Gen.int())) { fa, fb ->
      val fbEval: Eval<Kind<F, Int>> = Eval.just(fb)
      fa.followedByEval(fbEval).equalUnderTheLaw(fa.flatMap { fbEval.value() }, EQ)
    }

  private fun <F> Monad<F>.derivedProductLConsistent(GK: GenK<F>, EQ: Eq<Kind<F, Int>>): Unit =
    forAll(GK.genK(Gen.int()), GK.genK(Gen.int())) { fa, fb ->
      fa.productL(fb).equalUnderTheLaw(fa.flatMap { a -> fb.map { a } }, EQ)
    }

  private fun <F> Monad<F>.derivedProductLEvalConsistent(GK: GenK<F>, EQ: Eq<Kind<F, Int>>): Unit =
    forAll(GK.genK(Gen.int()), GK.genK(Gen.int())) { fa, fb ->
      val fbEval: Eval<Kind<F, Int>> = Eval.just(fb)
      fa.productLEval(fbEval).equalUnderTheLaw(fa.flatMap { a -> fbEval.value().map { a } }, EQ)
    }

  private fun <F> Monad<F>.derivedProductConsistent(GK: GenK<F>, AP: Apply<F>, EQ: Eq<Kind<F, Tuple2<Int, Int>>>): Unit =
    forAll(GK.genK(Gen.int()), GK.genK(Gen.int())) { fa, fb ->
      AP.run { fa.product(fb) }.equalUnderTheLaw(fa.product(fb), EQ)
    }

  private fun <F> Monad<F>.derivedMProductConsistent(GK: GenK<F>, EQ: Eq<Kind<F, Tuple2<Int, Int>>>): Unit =
  ...
  private fun <F> Monad<F>.derivedIfMConsistent(GK: GenK<F>, EQ: Eq<Kind<F, Int>>): Unit =
