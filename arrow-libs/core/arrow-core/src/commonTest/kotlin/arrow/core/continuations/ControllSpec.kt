      ...
      checkAll(Arb.int().orNull(), Arb.string()) { i: Int?, shift: String ->
        val res =
          either<String, Int> {
            ...
            square(ii) // Smart-cast by controllract
          }
...
internal suspend fun Throwable.suspend(): Nothing = suspendCoroutineUninterceptedOrReturn { control ->
  suspend { throw this }
    .startCoroutine(Continuation(Dispatchers.Default) { control.intercepted().resumeWith(it) })

  COROUTINE_SUSPENDED
}

internal suspend fun <A> A.suspend(): A = suspendCoroutineUninterceptedOrReturn { control ->
  suspend { this }
    .startCoroutine(Continuation(Dispatchers.Default) { control.intercepted().resumeWith(it) })
...
): A = suspendCancellableCoroutine { control ->
  control.invokeOnCancellation { cause ->
