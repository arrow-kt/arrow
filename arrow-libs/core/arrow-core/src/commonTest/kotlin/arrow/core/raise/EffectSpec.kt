...
class EffectSpec :
  StringSpec({
    ...
    "shift leaked results in ShiftLeakException with exception" {
      shouldThrow<ShiftLeakedException> {
        ...
        shouldThrow<RuntimeException> {
          ...
          }.fold({ fail("Cannot be here") }) { fail("Cannot be here") }
        }.message shouldBe "Boom"
        leak.await().invoke()
      }
    }

    "shift leaked results in ShiftLeakException after raise" {
      shouldThrow<ShiftLeakedException> {
        val leak = CompletableDeferred<suspend () -> Unit>()
        effect {
          leak.complete { raise("failure") }
          raise("Boom!")
        }.fold({ it shouldBe "Boom!" }) { fail("Cannot be here") }
        leak.await().invoke()
      }
    }
  })
