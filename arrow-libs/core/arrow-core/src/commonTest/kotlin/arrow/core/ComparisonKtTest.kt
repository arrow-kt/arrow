...
class ComparisonKtTest {
  ...
  @Test fun sortVararg() = runTest {
    checkAll(Arb.person(), Arb.list(Arb.person(), 0..20)) { a, list ->
      val aas = list.toTypedArray()
      val res = sort(a = a, aas = aas, comparator = Person.comparator)
      val expected = listOf(a, *aas).sorted()
