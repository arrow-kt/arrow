...
class ControlSpec : StringSpec({
  suspend fun <A> Control<Nothing, A>.value(): A = fold(::identity, ::identity)

  suspend fun Control<*, *>.runCont(): Any? = fold(::identity, ::identity)
  ...
  "immediate values" { control<Nothing, Int> { 1 }.value() shouldBe 1 }

  "suspended value" { control<Nothing, Int> { 1.suspend() }.value() shouldBe 1 }

  "immediate short-circuit" {
    control<String, Nothing> { shift("hello") }.runCont() shouldBe "hello"
  }

  "suspended short-circuit" {
    control<String, Nothing> { shift("hello".suspend()) }.runCont() shouldBe "hello"
  }

  "Rethrows immediate exceptions" {
    val e = RuntimeException("test")
    Either.catch { control<Nothing, Nothing> { throw e }.runCont() } shouldBe Either.Left(e)
  }

  "Rethrows suspended exceptions" {
    val e = RuntimeException("test")
    Either.catch { control<Nothing, Nothing> { e.suspend() }.runCont() } shouldBe Either.Left(e)
  }

  "Can short-circuit immediately from nested blocks" {
    control<String, Int> {
      control<Nothing, Long> { shift("test") }.runCont()
  ...
  "Can short-circuit suspended from nested blocks" {
    control<String, Int> {
      control<Nothing, Long> { shift("test".suspend()) }.runCont()
      fail("Should never reach this point")
    }
      .runCont() shouldBe "test"
  }
