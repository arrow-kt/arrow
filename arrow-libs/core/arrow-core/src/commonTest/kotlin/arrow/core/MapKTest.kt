...
class MapKTest {
  ...
  @Test fun alignedness() = runTest {
    checkAll(
      ...
      fun <A, B> Ior<A, B>.toLeftOption() = fold({ it }, { null }, { a, _ -> a })
  ...
  @Test @FlakyOnJs
  fun mapValuesOrAccumulateAccumulates() = runTest {
    checkAll(
  ...
  @Test fun mapValuesOrAccumulateEitherEmpty() = runTest {
    val result: Either<String, Map<Int, String>> = emptyMap<Int, Int>().mapValuesOrAccumulate(
      combine = { a, b -> "$a,$b" },
    ) {
      it.value.toString()
    }
    ...
    result
      .shouldBeInstanceOf<Either.Right<Map<Int, String>>>()
      .value.shouldBeEmpty()
  }

  @Test fun mapValuesOrAccumulateEitherMaps() = runTest {
    checkAll(
      Arb.map(Arb.int(), Arb.int(), maxSize = 30),
    ) { xs ->
      val result: Either<String, Map<Int, String>> = xs.mapValuesOrAccumulate(
        combine = { a, b -> "$a,$b" },
      ) {
        it.value.toString()
      }
      ...
      result
        .shouldBeInstanceOf<Either.Right<Map<Int, String>>>()
        .value shouldBe xs.mapValues { it.value.toString() }
    }
  }

  @Test
  fun mapValuesOrAccumulateEitherAccumulates() = runTest {
    checkAll(
      Arb.map(Arb.int(), Arb.int(), minSize = 1, maxSize = 30),
    ) { xs ->
      val separator = ","
      val result: Either<String, Map<Int, String>> = xs.mapValuesOrAccumulate(
        combine = { a, b -> "$a$separator$b" },
      ) {
      ...
      result
        .shouldBeInstanceOf<Either.Left<String>>()
