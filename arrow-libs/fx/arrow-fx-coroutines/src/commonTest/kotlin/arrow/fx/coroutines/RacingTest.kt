...
class RacingTest {
    ...
    fun testExceptionHandling() = runTest {
        ...
        val result = racing(handleException = { _, exception -> exceptionHandled.complete(exception) }) {
    ...
    fun testTimeout() = runTest {
        val result = racing {
            race { awaitCancellation() }
            onTimeout(time = 50.milliseconds) {
                "timeout"
            }
        }
    ...
    fun testNestedRacingErrorPropagation() = runTest {
        ...
        val result = racing(handleException = { _, exception -> outerExceptionHandled.complete(exception) }) {
            race {
                racing(handleException = { _, exception -> 
                    innerExceptionHandled.complete(exception)
                    innerRacingCompleted.complete(true)
                }) {
                    race { throw IllegalArgumentException("Inner exception") }
                    race { awaitCancellation() }
                }
    ...
    fun testMultipleRacersThrowingExceptions() = runTest {
        ...
        val result = racing(handleException = { _, exception ->
    ...
    fun testCancellationVsOtherExceptions() = runTest {
        ...
        val result = racing(handleException = { _, exception -> 
            ...
            race { throw RuntimeException("Non-cancellation exception") }
            race { 
                yield() // Give time for the other racers to throw
                "success" 
            }
