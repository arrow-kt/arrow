...
class RaiseRacingTest {
  ...
  fun testExceptionHandling() = runTest {
    ...
    val result = either<String, String> {
      racing(handleException = { _, exception -> exceptionHandled.complete(exception) }) {
        race { throw IllegalStateException("Test exception") }
        race { "success" }
      }
    }
  ...
  fun testRaiseHandling() = runTest {
    ...
    val result = either<String, String> {
      racing(handleRaise = { _, raised -> raiseHandled.complete(raised) }) {
  ...
  fun testNestedRacingErrorPropagation() = runTest {
    ...
    val result = either<String, String> {
      racing(handleException = { _, exception -> outerExceptionHandled.complete(exception) }) {
        race {
          racing(
            handleException = { _, exception -> innerExceptionHandled.complete(exception) },
            handleRaise = { _, raised ->
              innerRaiseHandled.complete(raised)
              innerRacingCompleted.complete(true)
            }
          ) {
            race { raise("Inner error") }
            race { awaitCancellation() }
          }
          ...
          innerRacingCompleted.complete(false)
          fail("inner completed")
        }
        race {
  ...
  fun testMultipleRacersThrowingExceptions() = runTest {
    ...
    val result = either<String, String> {
      racing(handleException = { _, exception ->
        exceptionsHandled.add(exception)
        if (exceptionsHandled.size >= 2) {
          exceptionHandlerCalled.complete(true)
        }
      }) {
        race { throw IllegalStateException("First exception") }
        race { throw RuntimeException("Second exception") }
        race {
  ...
  fun testMultipleRacersRaisingErrors() = runTest {
    ...
    assertEquals("success".right(), result)
    assertTrue(withTimeoutOrNull(600.milliseconds) { errorHandlerCalled.await() } == true)
  ...
  fun testMixedErrorsAndExceptions() = runTest {
    ...
    val result = either<String, String> {
      racing(
        handleRaise = { _, raised ->
          errorsHandled.add(raised)
          if (errorsHandled.size >= 1 && exceptionsHandled.size >= 1) {
        ...
        race { raise("Raised error") }
        race { throw RuntimeException("Thrown exception") }
        race {
    ...
    assertEquals("success".right(), result)
    assertTrue(withTimeoutOrNull(600.milliseconds) { handlersComplete.await() } == true)
