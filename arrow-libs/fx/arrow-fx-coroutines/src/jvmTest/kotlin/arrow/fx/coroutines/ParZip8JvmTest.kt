...
class ParZip8JvmTest {
  ...
  @Test fun parZip8ReturnsToOriginalContext(): Unit = runBlocking(Dispatchers.Default) {
  ...
  @Test fun parZip8ReturnsToOriginalContextOnFailure(): Unit = runBlocking(Dispatchers.Default) {
    val zipCtxName = "parZip8"
    resourceScope {
      val zipCtx = executor { Executors.newFixedThreadPool(8, NamedThreadFactory(zipCtxName)) }

      checkAll(10, Arb.int(1..8), Arb.throwable()) { choose, e ->
        withContext(single()) {
          threadName() shouldStartWith "single"
  ...
  @Test fun parZip8FinishesOnSingleThread(): Unit = runBlocking(Dispatchers.Default) {
