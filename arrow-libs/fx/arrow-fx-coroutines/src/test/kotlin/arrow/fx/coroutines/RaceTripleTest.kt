...
class RaceTripleTest : ArrowFxSpec(
  spec = {
        ...
        single.zip(racer).use { (single, raceCtx) ->
          withContext(single) {
            ...
            val racedOn = when (choose) {
              1 ->
                raceTriple(raceCtx, { threadName() }, { never<Nothing>() }, { never<Nothing>() })
                  .fold({ a, _, _ -> a }, { _, _, _ -> null }, { _, _, _ -> null })
              2 ->
                raceTriple(raceCtx, { never<Nothing>() }, { threadName() }, { never<Nothing>() })
                  .fold({ _, _, _ -> null }, { _, b, _ -> b }, { _, _, _ -> null })
              else ->
                raceTriple(raceCtx, { never<Nothing>() }, { never<Nothing>() }, { threadName() })
        ...
        single.zip(racer).use { (single, raceCtx) ->
          withContext(single) {
            ...
            Either.catch {
              when (choose) {
                1 ->
                  raceTriple(raceCtx, { e.suspend() }, { never<Nothing>() }, { never<Nothing>() })
                    .fold({ a, _, _ -> a }, { _, _, _ -> null }, { _, _, _ -> null })
                2 ->
                  raceTriple(raceCtx, { never<Nothing>() }, { e.suspend() }, { never<Nothing>() })
                    .fold({ _, _, _ -> null }, { _, b, _ -> b }, { _, _, _ -> null })
                else ->
                  raceTriple(raceCtx, { never<Nothing>() }, { never<Nothing>() }, { e.suspend() })
                    .fold({ _, _, _ -> null }, { _, _, _ -> null }, { _, _, c -> c })
