...
object TraversalLaws {
  ...
  fun <A, B> Traversal<A, B>.setIdempotent(aGen: Gen<A>, bGen: Gen<B>, EQA: (A, A) -> Boolean): Unit =
    forAll(aGen, bGen) { a, b ->
      set(set(a, b), b)
        .equalUnderTheLaw(set(a, b), EQA)
    }

  fun <A, B> Traversal<A, B>.modifyIdentity(aGen: Gen<A>, EQA: (A, A) -> Boolean): Unit =
    forAll(aGen) { a ->
      modify(a, ::identity).equalUnderTheLaw(a, EQA)
    }

  fun <A, B> Traversal<A, B>.composeModify(aGen: Gen<A>, funcGen: Gen<(B) -> B>, EQA: (A, A) -> Boolean): Unit =
