...
object PrismLaws {
  ...
  fun <A, B> Prism<A, B>.partialRoundTripOneWay(aGen: Gen<A>, EQA: (A, A) -> Boolean): Unit =
    forAll(aGen) { a ->
      getOrModify(a).fold(::identity, ::reverseGet)
        .equalUnderTheLaw(a, EQA)
    }

  fun <A, B> Prism<A, B>.roundTripOtherWay(bGen: Gen<B>, EQB: (B?, B?) -> Boolean): Unit =
    forAll(bGen) { b ->
      getOrNull(reverseGet(b))
        .equalUnderTheLaw(b, EQB)
    }

  fun <A, B> Prism<A, B>.modifyIdentity(aGen: Gen<A>, EQA: (A, A) -> Boolean): Unit =
    forAll(aGen) { a ->
      modify(a, ::identity).equalUnderTheLaw(a, EQA)
    }

  fun <A, B> Prism<A, B>.composeModify(aGen: Gen<A>, funcGen: Gen<(B) -> B>, EQA: (A, A) -> Boolean): Unit =
    forAll(aGen, funcGen, funcGen) { a, f, g ->
      modify(modify(a, f), g).equalUnderTheLaw(modify(a, g compose f), EQA)
    }

  fun <A, B> Prism<A, B>.consistentSetModify(aGen: Gen<A>, bGen: Gen<B>, EQA: (A, A) -> Boolean): Unit =
