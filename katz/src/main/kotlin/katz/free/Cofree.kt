...
typealias CofreeE<S, A> = HK<S, Cofree<S, A>>

fun <S, A> CofreeKind<S, A>.ev(): Cofree<S, A> = this as Cofree<S, A>

data class Cofree<S, A>(val FS: Functor<S>, val head: A, val tail: Eval<CofreeE<S, A>>) : CofreeKind<S, A> {
    ...
    fun tailForced(): CofreeE<S, A> =
            tail.value()

    inline fun <B> transform(f: (A) -> B, noinline g: (Cofree<S, A>) -> Cofree<S, B>) =
