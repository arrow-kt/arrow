 ...
 */
inline fun <A, B, reified C : A, D> Ior<A, B>.flatMap(aa: Semigroup<C>, crossinline f: (B) -> Ior<C, D>): Ior<C, D> = when (this) {
    is Ior.Left -> Ior.Left(value as C)
    is Ior.Right -> f(value)
    is Ior.Both -> {
        val fm = f(rightValue)
        when (fm) {
            is Ior.Left -> Ior.Left(aa.combine(leftValue as C, fm.value))
            is Ior.Right -> Ior.Right(fm.value)
            is Ior.Both -> Ior.Both(aa.combine(leftValue as C, fm.leftValue), fm.rightValue)
        }
    }
}
