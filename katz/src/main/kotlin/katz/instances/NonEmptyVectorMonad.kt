...
interface NonEmptyVectorMonad : Monad<NonEmptyVector.F> {
    override fun <A> pure(a: A): HK<NonEmptyVector.F, A> = TODO() //NonEmptyVector.of(a)

    override fun <A, B> flatMap(fa: HK<NonEmptyVector.F, A>, f: (A) -> HK<NonEmptyVector.F, B>): HK<NonEmptyVector.F, B> =
            fa.ev().flatMap { f(it).ev() }

    @Suppress("UNCHECKED_CAST")
    private tailrec fun <A, B> go(buf: Vector<B>, f: (A) -> HK<NonEmptyVector.F, Either<A, B>>, v: NonEmptyVector<Either<A, B>>): Unit = TODO()
//            when (v.head) {
//                is Either.Right<*> -> {
//                    buf += v.head.b as B
//                    val x = NonEmptyVector.fromArray(v.tail)
//                    when (x) {
...
//        val buf = Array<B>()
//        go(buf, f, f(a).ev())
//        return NonEmptyVector.fromArrayUnsafe(buf)
        return TODO()
    }
}
