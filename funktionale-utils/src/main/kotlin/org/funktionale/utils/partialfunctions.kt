...
infix fun <P1, R> PartialFunction<P1, R>.orElse(that: PartialFunction<P1, R>): PartialFunction<P1, R> = PartialFunction({ this.isDefinedAt(it) || that.isDefinedAt(it) }) {
        when {
...
fun <P1, R> ((P1) -> R).toPartialFunction(definedAt: (P1) -> Boolean): PartialFunction<P1, R> = PartialFunction(definedAt, this)