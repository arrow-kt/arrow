...
class ArrowGradlePlugin : Plugin<Project> {
  companion object {
    fun isEnabled(project: Project) =
      project.plugins.findPlugin(ArrowGradlePlugin::class.java) != null
...
class ArrowKotlinGradleSubplugin : KotlinGradleSubplugin<AbstractCompile> {
  companion object {
    ...
    private const val GENERATED_COMPILER_PLUGIN_ID = "arrow.meta.compiler.plugin"
    private const val PURE_CHECK_ARG_NAME = "pureCheck"
  }

  override fun isApplicable(project: Project, task: AbstractCompile) =
    ArrowGradlePlugin.isEnabled(project)
