...
interface EitherTMonadErrorInstance<F, L> : EitherTMonadInstance<F, L>, MonadError<EitherTKindPartial<F, L>, L> {
    ...
    override fun <A> raiseError(e: L): EitherT<F, L, A> = EitherT(MF().pure(Either.Left(e)))
}
...
interface EitherTFoldableInstance<F, L> : Foldable<EitherTKindPartial<F, L>> {
    ...
    override fun <B, C> foldL(fa: HK<EitherTKindPartial<F, L>, B>, b: C, f: (C, B) -> C): C = fa.ev().foldL(b, f, FFF())

    override fun <B, C> foldR(fa: HK<EitherTKindPartial<F, L>, B>, lb: Eval<C>, f: (B, Eval<C>) -> Eval<C>): Eval<C> = fa.ev().foldR(lb, f, FFF())
}
