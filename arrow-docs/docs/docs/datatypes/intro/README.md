...
`Option` behaves the same for `Int`, `String` or `DomainUserClass`. To indicate that `Option` is a [type constructor]({{ '/docs/patterns/glossary/' | relative_url }}) they implement the interface [`HK`]({{ '/docs/patterns/glossary/' | relative_url }}) it implements `OptionKind<A>`, which is a typealias of `HK<OptionHK, A>`.
