...
As you know, you're responsible for catching exceptions on coroutines. As you can see, implementation details like this catch,
or the observable subscriptions from previous examples, are related to the given data types being
used for each case. So, those must live here, in the program's edge.

The same result one more time:
...
There's also an alternative, fancier API provided by Arrow for [`DeferredK`]({{ '/docs/integrations/kotlinxcoroutines/' | relative_url }}). It takes care of runBlocking and waiting on the operations for you:
