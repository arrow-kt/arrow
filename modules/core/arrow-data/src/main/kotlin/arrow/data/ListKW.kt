...
data class ListK<out A> constructor(val list: List<A>) : ListKOf<A>, List<A> by list {
    ...
    fun <B> flatMap(f: (A) -> ListKOf<B>): ListK<B> = this.reify().list.flatMap { f(it).reify().list }.k()

    fun <B> map(f: (A) -> B): ListK<B> = this.reify().list.map(f).k()
    ...
    fun <B> ap(ff: ListKOf<(A) -> B>): ListK<B> = ff.reify().flatMap { f -> map(f) }.reify()
...
fun <A> ListK<A>.combineK(y: ListKOf<A>): ListK<A> = (this.list + y.reify().list).k()
