...
fun <A, T> ConstOf<A, T>.value(): A = this.extract().value
...
fun <A, T, U> ConstOf<A, T>.ap(ff: ConstOf<A, (T) -> U>, SG: Semigroup<A>): Const<A, U> = ff.extract().retag<U>().combine(this.extract().retag(), SG)
