 ...
 */
fun <F, S, A> StateTOf<F, S, A>.runM(MF: Monad<F>, initial: S): Kind<F, Tuple2<S, A>> = extract().run(initial, MF)
 ...
 * @param MF [Monad] for the context [F]
 */
inline fun <reified F, S, A> StateTOf<F, S, A>.runM(initial: S, MF: Monad<F> = monad()): Kind<F, Tuple2<S, A>> = extract().run(initial, MF)
...
class StateT<F, S, A>(
    ...
    fun <B> ap(ff: StateTOf<F, S, (A) -> B>, MF: Monad<F>): StateT<F, S, B> =
            ff.extract().map2(this, { f, a -> f(a) }, MF)
    ...
    fun combineK(y: StateTOf<F, S, A>, MF: Monad<F>, SF: SemigroupK<F>): StateT<F, S, A> =
            StateT(MF.pure({ s -> SF.combineK(run(s, MF), y.extract().run(s, MF)) }))
