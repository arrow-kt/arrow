...
fun <A, T, U> ConstOf<A, T>.apPipe(SG: Semigroup<A>, ff: ConstOf<A, (T) -> U>): Const<A, U> = fix().retag<U>().combine(SG, ff.fix().retag())
fun <A, T, U> ConstOf<A, (T) -> U>.ap(SG: Semigroup<A>, ff: ConstOf<A, T>): Const<A, U> = fix().retag<U>().combine(SG, ff.fix().retag())
