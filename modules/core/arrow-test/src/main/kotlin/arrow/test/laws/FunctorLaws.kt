...
object FunctorLaws {
  ...
  fun <F> laws(AP: Applicative<F>, EQ: Eq<Kind<F, Int>>): List<Law> =
    InvariantLaws.laws(AP, EQ) + listOf(
      Law("Functor Laws: Covariant Identity", { AP.covariantIdentity({ AP.just(it) }, EQ) }),
      Law("Functor Laws: Covariant Composition", { AP.covariantComposition({ AP.just(it) }, EQ) })
    )
