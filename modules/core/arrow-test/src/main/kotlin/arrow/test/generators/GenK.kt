...
fun <K> MapK.Companion.genK(kgen: Gen<K>) = object : GenK<MapKPartialOf<K>> {
  override fun <A> genK(gen: Gen<A>): Gen<Kind<MapKPartialOf<K>, A>> =
    Gen.mapK(kgen, gen) as Gen<Kind<MapKPartialOf<K>, A>>
}

fun <K : Comparable<K>> SortedMapK.Companion.genK(kgen: Gen<K>) = object : GenK<SortedMapKPartialOf<K>> {
