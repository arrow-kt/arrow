...
object AsyncLaws {
    inline fun <reified F> laws(AC: Async<F, Throwable> = async(), EQ: Eq<Kind<F, Int>>, EQ_EITHER: Eq<Kind<F, Either<Throwable, Int>>>, EQERR: Eq<Kind<F, Int>> = EQ): List<Law> =
    ...
    inline fun <reified F> asyncSuccess(AC: Async<F, Throwable> = async(), EQ: Eq<Kind<F, Int>>): Unit =
            forAll(Gen.int(), { num: Int ->
                AC.async { ff: (Either<Throwable, Int>) -> Unit -> ff(Right(num)) }.equalUnderTheLaw(AC.pure<Int>(num), EQ)
            })

    inline fun <reified F> asyncError(AC: Async<F, Throwable> = async(), EQ: Eq<Kind<F, Int>>): Unit =
