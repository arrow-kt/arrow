...
interface EitherFunctorInstance<L> : Functor<EitherPartialOf<L>> {
    override fun <A, B> map(fa: EitherOf<L, A>, f: (A) -> B): Either<L, B> = fa.reify().map(f)
}
...
interface EitherMonadInstance<L> : EitherApplicativeInstance<L>, Monad<EitherPartialOf<L>> {
    ...
    override fun <A, B> flatMap(fa: EitherOf<L, A>, f: (A) -> EitherOf<L, B>): Either<L, B> = fa.reify().flatMap { f(it).reify() }
