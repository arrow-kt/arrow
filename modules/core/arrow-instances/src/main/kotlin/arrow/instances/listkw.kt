...
interface ListKWFoldableInstance : Foldable<ForListKW> {
    override fun <A, B> foldLeft(fa: ListKWOf<A>, b: B, f: kotlin.Function2<B, A, B>): B =
            fa.reify().foldLeft(b, f)

    override fun <A, B> foldRight(fa: ListKWOf<A>, lb: Eval<B>, f: kotlin.Function2<A, Eval<B>, Eval<B>>): Eval<B> =
            fa.reify().foldRight(lb, f)

    override fun <A> isEmpty(fa: ListKWOf<A>): kotlin.Boolean =
            fa.reify().isEmpty()
}
...
interface ListKWTraverseInstance : Traverse<ForListKW> {
    ...
    override fun <G, A, B> traverse(fa: ListKWOf<A>, f: kotlin.Function1<A, Kind<G, B>>, GA: Applicative<G>): Kind<G, ListKW<B>> =
