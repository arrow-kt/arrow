...
sealed class Option<out A> : OptionKind<A> {
     ...
     * @see map
     */
    inline fun <B> flatMap(crossinline f: (A) -> OptionKind<B>): Option<B> = fold({ None }, { a -> f(a) }).reify()

    fun <B> ap(ff: OptionKind<(A) -> B>): Option<B> = ff.reify().flatMap { this.reify().map(it) }
 ...
 */
fun <A, B : A> OptionKind<B>.orElse(alternative: () -> Option<B>): Option<B> = if (reify().isEmpty()) alternative() else reify()

infix fun <T> OptionKind<T>.or(value: Option<T>): Option<T> = if (reify().isEmpty()) {
    value
} else {
    reify()
}
