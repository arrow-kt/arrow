...
class ScheduleTest : UnitSpec() {
  ...
  fun Schedule.Decision.Companion.genK(): GenK<DecisionPartialOf<Int>> = object : GenK<DecisionPartialOf<Int>> {
    override fun <A> genK(gen: Gen<A>): Gen<Kind<DecisionPartialOf<Int>, A>> =
      Gen.bind(
        Gen.bool(),
        Gen.intSmall(),
        gen
      ) { cont, delay, res -> Schedule.Decision(cont, delay.nanoseconds, 0, Eval.now(res)) }
  }
  ...
  init {
    ...
    "repeat" {
      forAll(Schedule.Decision.genK().genK(Gen.int()), Gen.intSmall().filter { it < 100 }.filter { it >= 0 }) { dec, n ->
        val schedule = Schedule(IO.monad(), IO.just(0)) { _: Unit, _ -> IO.just(dec.fix()) }
    ...
    "retry" {
      forAll(Schedule.Decision.genK().genK(Gen.int()), Gen.intSmall().filter { it < 100 }.filter { it >= 0 }) { dec, n ->
        ...
        val eff = SideEffect()
        val ref = Ref(IO.monadDefer<Nothing>(), 0.seconds).unsafeRunSync()
