...
class RefTest : UnitSpec() {
  ...
  init {
    ...
    "concurrent modifications" {
      val finalValue = 100
      val r = Ref.unsafe(0, IO.monadDefer<Nothing>())
      (0 until finalValue)
    ...
    "set get - successful" {
      forAll(Gen.int(), Gen.int()) { a, b ->
        Ref(IO.monadDefer<Nothing>(), a).flatMap { ref ->
          ref.set(b).flatMap {
            ref.get()
          }
    ...
    "consistent set update" {
      forAll(Gen.int(), Gen.int()) { a, b ->
        val set = Ref(IO.monadDefer<Nothing>(), a).flatMap { ref -> ref.set(b).flatMap { ref.get() } }
        val update = Ref(IO.monadDefer<Nothing>(), a).flatMap { ref -> ref.update { b }.flatMap { ref.get() } }
    ...
    "consistent access tryModify" {
      forAll(Gen.int(), Gen.functionAToB<Int, Int>(Gen.int())) { a, f ->
        val accessMap = Ref(IO.monadDefer<Nothing>(), a).flatMap { ref -> ref.access().map { (a, setter) -> setter(f(a)) } }.flatten()
        val tryUpdate = Ref(IO.monadDefer<Nothing>(), a).flatMap { ref -> ref.tryUpdate(f) }
