...
class IOTest : UnitSpec() {
  ...
  init {
    ...
    "IOParMap2 left handles null" {
      parMapN(just<Int?>(null), IO.unit) { _, unit -> unit }
        .unsafeRunSync() shouldBe Unit
    }

    "IOParMap2 right handles null" {
      parMapN(IO.unit, IO.just<Int?>(null)) { unit, _ -> unit }
        .unsafeRunSync() shouldBe Unit
    }

    "IOParMap3 left handles null" {
      parMapN(just<Int?>(null), IO.unit, IO.unit) { _, unit, _ -> unit }
        .unsafeRunSync() shouldBe Unit
    }

    "IOParMap3 middle handles null" {
      parMapN(IO.unit, IO.just<Int?>(null), IO.unit) { unit, _, _ -> unit }
        .unsafeRunSync() shouldBe Unit
    }

    "IOParMap3 right handles null" {
      parMapN(IO.unit, IO.unit, IO.just<Int?>(null)) { unit, _, _ -> unit }
        .unsafeRunSync() shouldBe Unit
    }
