...
interface WriterTApplicative<F, W> : Applicative<WriterTPartialOf<F, W>>, WriterTFunctor<F, W> {
  ...
  override fun <A, B> Kind<WriterTPartialOf<F, W>, A>.lazyAp(ff: Eval<Kind<WriterTPartialOf<F, W>, (A) -> B>>): Eval<Kind<WriterTPartialOf<F, W>, B>> =
    WriterT(
      AF().run {
        fix().value().lazyAp(ff.map { fb ->
          fb.fix().value().map { (w2, f) ->
            { (w1, a): Tuple2<W, A> ->
              MM().run { w1 + w2 } toT f(a)
            }
          }
        }).value()
      }
