...
class KleisliTest : UnitSpec() {
  private fun <A> TryEQ(): Eq<KleisliOf<Int, ForTry, A>> = Eq { a, b ->
    a.run(1) == b.run(1)
  }
  ...
  private fun IOEQ(): Eq<Kind<KleisliPartialOf<Int, ForIO>, Int>> = Eq { a, b ->
    a.run(1).attempt().unsafeRunSync() == b.run(1).attempt().unsafeRunSync()
  }

  private fun IOEitherEQ(): Eq<Kind<KleisliPartialOf<Int, ForIO>, Either<Throwable, Int>>> = Eq { a, b ->
    a.run(1).attempt().unsafeRunSync() == b.run(1).attempt().unsafeRunSync()
  }
  ...
  init {
    ...
    testLaws(
      ...
      BracketLaws.laws(
        Kleisli.bracket<Int, ForIO, Throwable>(IO.bracket()),
        EQ = IOEQ(),
      ...
      MonadErrorLaws.laws(Kleisli.monadError<Int, ForTry, Throwable>(Try.monadError()), TryEQ(), TryEQ())
    )
