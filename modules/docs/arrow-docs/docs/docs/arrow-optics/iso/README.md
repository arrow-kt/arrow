...
pointIsoTuple.modifyF(Option.functor(), point) {
    Some((tuple.a / 2) toT (tuple.b / 2))
}
```

```kotlin:ank
val liftF: (Point2D) -> OptionOf<Point2D> = pointIsoTuple.liftF(Option.functor()) {
    try { Some((tuple.a / 2) toT (tuple.b / 0)) } catch(e:Throwable) { None }
}
