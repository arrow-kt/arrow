...
val ioRef: IO<Throwable, Ref<ForIO, Int>> = Ref(IO.monadDefer()) { 1 }.fix()
```
...
val ref1: IO<Throwable, Ref<ForIO, String>> = ref.later { "Hello, World!" }.fix()
val ref2: IO<Throwable, Ref<ForIO, Int>> = ref.later { 2 }.fix()
```
