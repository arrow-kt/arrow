...
class DataTypeExamples : FreeSpec() { init {
  ...
  "Either left or right" - {
    fun parse(s: String): ProblemOrInt = try { Either.right(s.toInt()) } catch(e: Throwable) { Either.left(invalidInt) }
    ...
    "Functor" {
      ...
      val actual = Either.functor<Int>().run { (try { Either.right("3".toInt()) } catch(e: Throwable) { Either.left(e) }).map { it + 1 } }
  ...
  "Try and recover" - {
    ...
    "Recover" {
      ...
      gain.handleErrorWith { try { Either.Right(playLottery ( 42)) } catch(e: Throwable) { Either.Left(e)} } shouldBe Either.Right(1000)
    }
  }
}
