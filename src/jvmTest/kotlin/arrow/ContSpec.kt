...
class ContSpec :
  StringSpec({
    ...
    "immediate values" { cont<Nothing, Int> { 1 }.value() shouldBe 1 }

    "suspended value" { cont<Nothing, Int> { 1.suspend() }.value() shouldBe 1 }

    "immediate short-circuit" {
      cont<String, Nothing> { shift("hello") }.runCont() shouldBe "hello"
    }

    "suspended short-circuit" {
      cont<String, Nothing> { shift("hello".suspend()) }.runCont() shouldBe "hello"
    }

    "Rethrows immediate exceptions" {
      val e = RuntimeException("test")
      Either.catch { cont<Nothing, Nothing> { throw e }.runCont() } shouldBe Either.Left(e)
    }

    "Rethrows suspended exceptions" {
      val e = RuntimeException("test")
      Either.catch { cont<Nothing, Nothing> { e.suspend() }.runCont() } shouldBe Either.Left(e)
    }
